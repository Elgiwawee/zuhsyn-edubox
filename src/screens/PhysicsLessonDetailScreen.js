...
const PhysicsLessonDetailScreen = () => {
  const route = useRoute();
  const navigation = useNavigation();
  const { topicIndex } = route.params ?? {};
  const lesson = physicsLessons?.[topicIndex];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ✅ Mark lesson completed (for progress)
      await markLessonCompleted(
        user.id,
        'physics',   // subject key (string)
