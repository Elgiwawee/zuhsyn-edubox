...
import { mathsLessons } from '../data/mathsLessons';
...
const MathsLessonDetailScreen = () => {
  const route = useRoute();
  const navigation = useNavigation();
  const { topicIndex } = route.params ?? {};
  const lesson = mathsLessons?.[topicIndex];
  ...
  const checkIfTaken = async () => {
    ...
    try {
      ...
      await markLessonCompleted(
        user.id,
        'mathematics',   // subject key (string)
